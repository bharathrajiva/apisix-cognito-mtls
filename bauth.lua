...
local schema = {
    type = "object",
    properties = {
        region = { type = "string", default = "aws-region-id" },
        pool_id = { type = "string", default = "aws-cognito-pool-id" },
        white_list = {
            type = "array",
            items = {
                type = "object",
...
local function extract_group_from_jwt(conf, decoded_payload)
    if decoded_payload and decoded_payload.payload and decoded_payload.payload["cognito:groups"] then
        local groups = decoded_payload.payload["cognito:groups"]
        local allowed_groups = conf.groups
        local seen_groups = {}
        local groups_string = ""
        local is_allowed_group = false

        for _, group in ipairs(groups) do
            if not seen_groups[group] then
                seen_groups[group] = true
                if groups_string ~= "" then
        ...
        return groups_string, is_allowed_group
    end
    return "", false  
end
...
local function auth(conf, ctx)
    ...
    if ex then
        return get_unauthorized_response("AWS Cognito JWK signature verification failed.")
    end
    ...
    -- Now we check the Cognito group
    local groups_string, is_admin = extract_group_from_jwt(conf, decoded_payload)

    if not is_admin then
        return get_unauthorized_response("Access Denied: Your current permission level does not allow entry to this route.")
    end

    core.request.set_header(ctx, "X-UserProfile", core.json.encode(res.payload))
    core.request.set_header(ctx, "X-UserName", decoded_payload.payload["username"])
    core.request.set_header(ctx, "X-Groups", groups_string)
end
